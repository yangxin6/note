...
一般我们会把$W$的这$n$个特征向量标准化，即满足$||\omega_i||_2=1, ||w_i||_2=1$, 或者说$\omega^T_i\omega_i=1$，此时$W$的$n$个特征向量为标准==正交基==，满足$W^TW=I$，即$W^T=W^{−1}$, 也就是说$W$为==酉矩阵==。这样我们的特征分解表达式可以写成:
...
其中U是一个$m×m$的矩阵， $\Sigma$ 是一个$m×n$的矩阵，除了主对角线上的元素以外全为0，==主对角线==上的每个元素都称为==奇异值==，$V$是一个$n×n$的矩阵。$U$和$V$都是==酉矩阵==，即满足 $U^TU=I,V^TV=I,U^TU=I,V^TV=I$。下图可以很形象的看出上面SVD的定义：
